...
########## Couleuvre tachetee - Lampropeltis triangulum ##########
#### Load packages ####
# ------------------- #
...
# ------------------ #
## --> DL data 
# obs <- get_EMV_qc("Lampropeltis triangulum"); dim(obs)
# saveRDS(obs, "/home/claire/BDQC-GEOBON/data/occurences/REPT_couleuvre_tachetee_occ.rds")

obs <- readRDS("/home/claire/BDQC-GEOBON/data/occurences/REPT_couleuvre_tachetee_occ.rds")
dim(obs)
table(obs$year_obs)
...
points(occs)
plot(occs_buf, border = "darkgrey", lwd = 3, add = TRUE)
...
# Zoom
occs_buf2 <- sf::st_buffer(occs_sf_utm, dist = 100000) %>%
  sf::st_union() %>% 
  sf::st_sf() %>%
  sf::st_transform(crs = raster::crs(bioclim_qc))


zoom_temp <- raster::crop(maxL@predictions[[1]], occs_buf2)
zoom_pred_temp <- raster::mask(zoom_temp, occs_buf2)

zoom_prec <- raster::crop(maxL@predictions[[2]], occs_buf2)
zoom_pred_prec <- raster::mask(zoom_prec, occs_buf2)


x11(); par(mfrow = c(1, 2))
plot(zoom_pred_temp); points(occs)
plot(zoom_pred_prec); points(occs)
